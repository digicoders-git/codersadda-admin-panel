      ...
      try {
        ...
        if (res.success && res.data) {
          ...
          setFormData({
            ...
            benefits: Array.isArray(sub.planBenefits) ? sub.planBenefits.join(", ") : "",
            badge: sub.planPricingType || "normal",
            status: sub.planStatus ? "Active" : "Disabled",
          });
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = {
        ...
        planPricingType: Number(formData.price) > 0 ? "paid" : "free",
        planBenefits: formData.benefits,
        planStatus: formData.status === "Active",
      };

      const res = await apiUpdateSubscription(id, data);
      if (res.success) {
