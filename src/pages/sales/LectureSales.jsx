...
function LectureSales() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error(err);
        toast.error("Failed to load lecture sales data");
      } finally {
        setLoading(false);
      }
    };
  ...
  const processTrends = () => {
    ...
    return {
      categories: raw.map((t) => {
        if (filterType === "monthly" || filterType === "allTime") {
        ...
        if (filterType === "yearly") return t._id;
  ...
  const lineChartOptions = {
    ...
    title: { text: "Lecture Sales Trends", style: { color: colors.text } },
    xAxis: {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p style={{ color: colors.textSecondary }} className="text-sm mt-1">
            Monitor revenue and enrollments for individual paid lectures
          </p>
        </div>
